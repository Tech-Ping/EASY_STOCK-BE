...
public interface MemberQuizRepository extends JpaRepository<MemberQuiz, Long> {
    MemberQuiz findByMemberIdAndQuizId(Long memberId, Long quizId);

    @Query("SELECT q FROM Quiz q " +
            "WHERE q.levelType = :levelType " +
            "AND NOT EXISTS (SELECT 1 FROM MemberQuiz mq WHERE mq.quiz.id = q.id AND mq.member.id = :memberId AND mq.completed = true)")
    ...
    boolean existsByMemberIdAndQuizId(Long memberId, Long id);
}
